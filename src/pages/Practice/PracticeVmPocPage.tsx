...
import { advanceCompletionFeedback, enqueueCompletionFeedback, type CompletionFeedbackItem, type CompletionFeedbackQueueState, } from '../../features/progress/completionFeedbackQueue';
...
import { evaluateMissionWithVmSnapshot, parseProbeMetricFromLine, parseTmuxActionsFromCommand, stripAnsi, type VmBridgeSnapshot, type VmProbeMetric, } from '../../features/vm/missionBridge';
import { createTmuxShortcutTelemetryState, parseTmuxShortcutTelemetry, } from '../../features/vm/tmuxShortcutTelemetry';
import { PROBE_LOOP_START_COMMAND, PROBE_LOOP_STOP_COMMAND, PROBE_TRIGGER_COMMAND, SEARCH_PROBE_TRIGGER_COMMAND, buildTerminalGeometrySyncCommand, } from './probeCommands';
import { buildLessonProgressRows, filterLessonProgressRows, resolveDefaultMissionSlugForLesson, type LessonCompletionStatus, type LessonFilter, } from './lessonProgress';
...
const TERMINAL_GEOMETRY_SYNC_COMMAND = buildTerminalGeometrySyncCommand(DEFAULT_TERMINAL_COLS, DEFAULT_TERMINAL_ROWS);
const QUICK_COMMANDS = [
    ...
    {
        label: __tx("\uC0C8 \uC708\uB3C4\uC6B0 \uC0DD\uC131"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux new-window -t lesson -n win2; tmux list-windows -t lesson',
    },
    {
        label: __tx("Pane \uBD84\uD560"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux split-window -t lesson; tmux list-panes -t lesson',
    },
    {
        label: __tx("\uB2E4\uC74C \uC708\uB3C4\uC6B0"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux next-window -t lesson; tmux display-message -p "window #{window_index}"',
    },
    {
        label: __tx("Copy Search \uC131\uACF5"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux copy-mode -t lesson:0.0; tmux send-keys -t lesson:0.0 -X search-backward "bin"; printf "[[TMUXWEB_PROBE:search:1]]\\n[[TMUXWEB_PROBE:searchMatched:1]]\\n" > /dev/ttyS1',
    },
    {
        label: __tx("Copy Search \uC2E4\uD328"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux copy-mode -t lesson:0.0; tmux send-keys -t lesson:0.0 -X search-backward "__TMUXWEB_NOT_FOUND__"; printf "[[TMUXWEB_PROBE:search:1]]\\n[[TMUXWEB_PROBE:searchMatched:0]]\\n" > /dev/ttyS1',
    },
    {
        label: __tx("\uB808\uC774\uC544\uC6C3 \uBCC0\uACBD"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux select-layout -t lesson:0 even-horizontal; tmux display-message -p "#{window_layout}"',
    },
    {
        label: __tx("Pane \uC90C \uD1A0\uAE00"),
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux resize-pane -t lesson:0.0 -Z; tmux display-message -p "#{window_zoomed_flag}"',
    },
    {
        label: 'Pane Sync ON',
        command: 'tmux has-session -t lesson 2>/dev/null || tmux new-session -d -s lesson; tmux set-window-option -t lesson:0 synchronize-panes on; tmux show-window-options -t lesson:0 -v synchronize-panes',
    },
] as const;
const LEARNING_PATH_ENTRY_LESSON = 'hello-tmux';
const LESSON_FILTER_OPTIONS: Array<{
    value: LessonFilter;
    label: string;
}> = [
    { value: 'all', label: __tx("\uC804\uCCB4") },
    { value: 'continue', label: __tx("\uC774\uC5B4\uD558\uAE30") },
    { value: 'incomplete', label: __tx("\uBBF8\uC644\uB8CC") },
    { value: 'completed', label: __tx("\uC644\uB8CC") },
];
...
const VM_BOOT_CONFIG = {
    ...
    initialStatePath: (import.meta.env.VITE_VM_INITIAL_STATE_PATH as string | undefined)?.trim() ||
...
const SHORTCUT_TOOLTIP_TEXT = __tx("\uC785\uB825 \uC21C\uC11C: Ctrl \uD0A4\uB97C \uB204\uB978 \uCC44 b\uB97C \uB204\uB978 \uB4A4 \uC190\uC744 \uB5BC\uACE0, \uB2E4\uC74C \uD0A4\uB97C \uC785\uB825\uD558\uC138\uC694.");
...
function isLikelyCommandText(value: string) {
    ...
    return /(new-window|split-window|resize-pane|choose-tree|command-prompt|next-window|previous-window|attach)/.test(normalized);
}
...
function getInitialScenarioLabel(initialScenario: string) {
    switch (initialScenario) {
        case 'single-pane':
            return __tx("\uCD08\uAE30 \uC2DC\uB098\uB9AC\uC624: \uB2E8\uC77C pane\uC5D0\uC11C \uC2DC\uC791");
        case 'log-buffer':
...
export function PracticeVmPocPage() {
    ...
    const [vmStatusText, setVmStatusText] = useState(__tx("\uB300\uAE30 \uC911"));
    ...
    const filteredLessonRows = useMemo(() => filterLessonProgressRows(lessonProgressRows, lessonFilter), [lessonFilter, lessonProgressRows]);
    ...
    const selectLessonForAction = useCallback((lessonSlug: string, options?: {
        resetFilter?: boolean;
    }) => {
        if (!content) {
            return;
    ...
    const announceAchievements = useCallback((achievementIds: string[]) => {
        ...
        const definitions = uniqueIds
            ...
            .filter((definition): definition is NonNullable<ReturnType<typeof getAchievementDefinition>> => Boolean(definition));
        if (definitions.length === 0) {
            return;
        }
        const unseenDefinitions = definitions.filter((definition) => !seenCelebrationKeysRef.current.has(`achievement:${definition.id}`));
    ...
    const updateMetricByProbe = useCallback((metric: VmProbeMetric) => {
        setVmStatus('running');
        setVmStatusText(__tx("\uBD80\uD305 \uC644\uB8CC, \uBA85\uB839 \uC785\uB825 \uAC00\uB2A5"));
        setMetrics((previous) => {
            switch (metric.key) {
                case 'session':
                    return {
    ...
    const registerCommand = useCallback((command: string, options?: {
    ...
    const completeSelectedMission = useCallback((mode: 'auto' | 'manual') => {
        ...
        const lessonWasCompleted = lessonMissions.length > 0 && lessonMissions.every((mission) => completedSetBefore.has(mission.slug));
        const lessonNowCompleted = lessonMissions.length > 0 && lessonMissions.every((mission) => completedSetAfter.has(mission.slug));
    ...
    const captureInteractiveCommandInput = useCallback((data: string) => {
    ...
    const sendCommand = useCallback((command: string, options?: {
    ...
    useEffect(() => {
        ...
        setVmStatus('booting');
        setVmStatusText(__tx("v86 \uCD08\uAE30\uD654 \uC911"));
        const host = terminalHostRef.current;
        if (!host) {
            setVmStatus('error');
            setVmStatusText(__tx("\uD130\uBBF8\uB110 DOM \uCD08\uAE30\uD654 \uC2E4\uD328"));
            return undefined;
        }
        ...
        (async () => {
            try {
                ...
                setVmStatusText(__tx("VM \uC2DC\uC791 \uC774\uBBF8\uC9C0 \uD655\uC778 \uC911..."));
                ...
                if (useWarmStart) {
                    setVmStatusText(__tx("\uBE60\uB978 \uC2DC\uC791 \uC2A4\uB0C5\uC0F7 \uB85C\uB529 \uC911..."));
                }
                ...
                if (useWarmStart) {
                    try {
                        ...
                        useWarmStart = false;
                        setVmStatusText(__tx("\uBE60\uB978 \uC2DC\uC791 \uC2A4\uB0C5\uC0F7 \uBCF5\uC6D0 \uC2E4\uD328, \uC77C\uBC18 \uBD80\uD305\uC73C\uB85C \uC804\uD658"));
                ...
                loadedListener = () => {
                    setVmStatusText(useWarmStart ? __tx("\uBE60\uB978 \uC2DC\uC791 \uC2A4\uB0C5\uC0F7 \uBCF5\uC6D0 \uC644\uB8CC") : __tx("\uCEE4\uB110 \uBC0F \uB8E8\uD2B8FS \uB85C\uB529 \uC644\uB8CC"));
                };
                stopListener = () => {
                    setVmStatus('stopped');
                    setVmStatusText(__tx("VM\uC774 \uC911\uC9C0\uB418\uC5C8\uC2B5\uB2C8\uB2E4"));
                };
                ...
                setVmStatus('error');
                setVmStatusText(__tx("v86 \uCD08\uAE30\uD654 \uC2E4\uD328 (bios/wasm \uACBD\uB85C \uD655\uC778 \uD544\uC694)"));
            }
        })();
        return () => {
            isMounted = false;
    ...
    if (contentState.status === 'loading') {
        return (<PagePlaceholder eyebrow="Practice" title={__tx("Browser VM \uCD08\uAE30\uD654 \uC911")} description={__tx("\uCEE4\uB9AC\uD058\uB7FC\uACFC VM \uB9AC\uC18C\uC2A4\uB97C \uB85C\uB529\uD558\uACE0 \uC788\uC2B5\uB2C8\uB2E4.")}/>);
    }
    if (contentState.status === 'error' || !content) {
        return (<PagePlaceholder eyebrow="Practice" title={__tx("VM Practice \uB85C\uB4DC \uC2E4\uD328")} description={__tx("\uCEE4\uB9AC\uD058\uB7FC \uB370\uC774\uD130\uB97C \uC77D\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.")}>
    ...
    return (<PagePlaceholder eyebrow="Practice" title={__tx("tmux \uC2E4\uC2B5")} description="">
      <div className="vm-poc-panel">
        ...
        {celebration ? (<section className={`vm-celebration-overlay ${isAchievementCelebration ? 'is-toast' : ''}`}>
            <section className={`vm-celebration vm-celebration-${celebration.kind} ${isAchievementCelebration ? 'is-toast' : ''}`} role={isAchievementCelebration ? 'status' : 'dialog'} aria-modal={isAchievementCelebration ? undefined : true} aria-live="polite" aria-label={__tx("\uC644\uB8CC \uD53C\uB4DC\uBC31")}>
              ...
              {celebration.kind === 'mission' && nextIncompleteMission ? (<section className="vm-celebration-next-action">
        ...
        <section className={`vm-workbench vm-workbench-view-${mobileWorkbenchView}`}>
          <aside className="vm-study-panel">
            {selectedMission ? (<article className="vm-mission-card vm-mission-priority-card">
                ...
                <section className="vm-mission-command-block">
                  ...
                  {selectedMissionCommands.length > 0 ? (<div className="vm-mission-command-list">
                    ...
                    </div>) : (<p className="muted">{__tx("\uCD94\uCC9C \uBA85\uB839\uC744 \uCC3E\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4. \uC544\uB798 \uD78C\uD2B8\uB97C \uAE30\uC900\uC73C\uB85C \uC9C1\uC811 \uC785\uB825\uD574 \uC8FC\uC138\uC694.")}</p>)}
                  <p className="muted">{__tx("\uBA85\uB839\uC744 \uD074\uB9AD\uD558\uBA74 \uC785\uB825\uCC3D\uC5D0 \uCC44\uC6CC\uC9C0\uACE0 \uD130\uBBF8\uB110 \uD0ED\uC73C\uB85C \uC804\uD658\uB429\uB2C8\uB2E4.")}</p>
                </section>

                <section className="vm-mission-precondition-block">
                  <h3>{__tx("\uC2E4\uD589 \uC804 \uD504\uB9AC\uCEE8\uB514\uC158")}</h3>
                  <ul className="vm-precondition-list">
                    {selectedMissionPreconditions.map((item) => (<li key={item.key} className={`vm-precondition-row ${item.satisfied ? 'is-satisfied' : 'is-pending'}`}>
            ...
            <section className="vm-next-action-card">
              ...
              <h2>{__tx("\uC9C0\uAE08 \uD560 \uC77C")}</h2>
              {!selectedMission ? (<p className="muted">{__tx("\uBBF8\uC158\uC744 \uC120\uD0DD\uD558\uBA74 \uBC14\uB85C \uC2E4\uD589\uD560 \uB2E4\uC74C \uD589\uB3D9\uC744 \uC548\uB0B4\uD569\uB2C8\uB2E4.")}</p>) : null}
            ...
            if (isCompleted) {
                ...
                badgeLabel = __tx("\uC2E4\uC2DC\uAC04 \uD1B5\uACFC");
            }
            else if (missionStatus?.status === 'manual') {
                badgeClass = 'is-manual';
                badgeLabel = __tx("\uC218\uB3D9");
            }
            return (<button key={mission.id} type="button" className={`vm-mission-row ${isSelected ? 'is-active' : ''}`} onClick={() => selectMissionForAction(mission.slug)}>
            ...
            {selectedMissionSession ? (<section className="vm-session-card">
                <p className="vm-session-card-eyebrow">{__tx("\uD604\uC7AC \uC138\uC158 \uC0C1\uD0DC")}</p>
                <p>
                  <strong>
            ...
            <section className="vm-curriculum-panel vm-curriculum-row-layout">
              ...
              <section className="vm-lesson-catalog" aria-label={__tx("\uB808\uC2A8 \uBAA9\uB85D")}>
                {filteredLessonRows.length === 0 ? (<p className="muted">{__tx("\uC120\uD0DD\uD55C \uD544\uD130\uC5D0 \uD574\uB2F9\uD558\uB294 \uB808\uC2A8\uC774 \uC5C6\uC2B5\uB2C8\uB2E4.")}</p>) : (filteredLessonRows.map((row) => {
            ...
            return (<button key={row.lesson.id} type="button" className={`vm-lesson-row ${isActive ? 'is-active' : ''}`} onClick={() => selectLessonForAction(row.lesson.slug)} aria-pressed={isActive}>
          ...
          <section className="vm-lab-panel">
            ...
            sendCommand(commandInput);
              ...
              <details className="vm-poc-quick-wrap">
                <summary>{__tx("\uC790\uC8FC \uC4F0\uB294 \uBA85\uB839 \uBE60\uB974\uAC8C \uC2E4\uD589")}</summary>
