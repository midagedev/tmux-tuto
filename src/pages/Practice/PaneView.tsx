...
export function PaneView({ pane, isActive, mouseEnabled, copyMatchLineSet, activeMatchLine, onFocusPane, onScrollPane, }: PaneViewProps) {
    ...
    return (<div className={`sim-pane-card${isActive ? ' is-active' : ''}`} data-active={isActive ? 'true' : 'false'} aria-label={`Pane ${pane.id}${isActive ? ' active' : ''}, x ${pane.x} y ${pane.y} width ${pane.width} height ${pane.height}`} onClick={() => {
            if (!mouseEnabled) {
                return;
            }
            handleFocus();
        }} onKeyDown={handleKeyDown} onWheel={handleWheel} role="button" tabIndex={0}>
      ...
      <div className={`sim-pane-body${isActive ? ' is-active' : ''}`}>
        ...
        {viewportLines.length === 0 ? (<div className="sim-pane-line">(empty)</div>) : (viewportLines.map((line, index) => {
            ...
            return (<div key={line.id} className={`sim-pane-line${isMatch ? ' is-match' : ''}${isActiveMatch ? ' is-active-match' : ''}`}>
                {line.text || ' '}
              </div>);
        }))}
      </div>
