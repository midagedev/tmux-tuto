              ...
              {orderedLessons.map((lesson) => {
                return (
                  <div key={lesson.id} className="curriculum-lesson-row">
                    ...
                    <p className="curriculum-lesson-meta">
                      <span>{t('예상 {{minutes}}분', { minutes: lesson.estimatedMinutes })}</span>
                      <span>{t('학습 목표 {{count}}개', { count: lesson.objectives.length })}</span>
                    </p>
                    {lesson.overview ? <p className="curriculum-lesson-overview">{t(lesson.overview)}</p> : null}
                  </div>
                );
              })}
            </div>
