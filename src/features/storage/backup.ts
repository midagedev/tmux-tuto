...
export async function importAllData(rawPayload: unknown, mode: ImportMode) {
  ...
  await Promise.all(
    parsed.stores.mission_attempts.map((row) => tx.objectStore('mission_attempts').put(row)),
  );
  ...
  await Promise.all(
    parsed.stores.simulator_snapshots.map((row) => tx.objectStore('simulator_snapshots').put(row)),
  );
