...
import type { AchievementRecord, BackupMetaRecord, BookmarkRecord, MissionAttemptRecord, NoteRecord, ProfileRecord, ProgressRecord, SimulatorSnapshotRecord, } from './types';
const STORE_NAMES = [
    'profile',
    'progress',
    'mission_attempts',
...
export async function clearAllData() {
    ...
    await Promise.all(STORE_NAMES.map((storeName: StoreName) => tx.objectStore(storeName).clear()));
    await tx.done;
}
