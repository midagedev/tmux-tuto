...
const DEFAULT_TARGET: AchievementShareTarget = {
  path: '/practice/hello-tmux?mission=hello-tmux-version-check',
...
const ACHIEVEMENT_TARGETS: Record<string, AchievementShareTarget> = {
  ...
  workspace_bootstrap: {
    path: '/practice/basics?mission=session-create',
    challengeLabel: '기본 조작: Session/Window/Pane',
  },
  copy_mode_starter: {
    path: '/practice/copy-search?mission=copy-mode-search-keyword',
    challengeLabel: 'Copy Mode 검색',
  },
  command_flow_starter: {
    path: '/practice/command-subset?mission=command-mode-prompt-intro',
    challengeLabel: 'Command Mode 운영',
  },
  track_a_completed: {
    path: '/practice/split-resize?mission=split-two-panes',
  ...
  full_curriculum_completed: {
    path: '/practice/command-subset?mission=command-mode-tree-intro',
    challengeLabel: '완주자 챌린지: choose-tree 탐색',
  },
  streak_7_days: {
    path: '/practice/attach-detach?mission=session-attach-reconnect',
    challengeLabel: '세션 유지 루틴',
  },
  lesson_explorer: {
    path: '/practice/pane-focus-flow?mission=pane-grid-layout',
  ...
  layout_alchemist: {
    path: '/practice/split-resize?mission=split-resize-adjust',
    challengeLabel: '레이아웃 조정 챌린지',
  },
  focus_navigator: {
    path: '/practice/pane-focus-flow?mission=window-cycle-practice',
