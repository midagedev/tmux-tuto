import { createInitialSimulatorState, createPane, createSession, createWindow, getActiveShellSession, getActiveSession, getActiveWindow, type ShellSession, type TmuxPane, type SimulatorMode, type SimulatorState, } from './model';
...
export type SimulatorAction = {
    ...
    type: 'SET_MODE';
    payload: SimulatorMode;
} | {
    type: 'ENTER_PREFIX_PENDING';
    payload: {
        at: number;
    };
} | {
    type: 'SET_REPEAT_WINDOW';
    payload: number | null;
} | {
...
function mapSessionWindows(state: SimulatorState, fn: (window: ReturnType<typeof getActiveWindow>) => ReturnType<typeof getActiveWindow>) {
    return state.tmux.sessions.map((session) => {
        ...
        return {
            ...session,
            windows: session.windows.map((window) => window.id === session.activeWindowId ? fn(window as ReturnType<typeof getActiveWindow>) : window),
        };
    });
}
function mapShellSessions(state: SimulatorState, fn: (session: ShellSession) => ShellSession) {
    return state.shell.sessions.map((session) => session.id === state.shell.activeSessionId ? fn(session) : session);
}
...
function withActivePane(state: SimulatorState, fn: (pane: TmuxPane) => TmuxPane): SimulatorState {
...
export function simulatorReducer(state: SimulatorState, action: SimulatorAction): SimulatorState {
    switch (action.type) {
        case 'SET_PREFIX_KEY': {
            ...
            }, 'sim.prefix.set', `Prefix set to ${action.payload}`);
        }
        case 'SET_MODE': {
            return {
                ...state,
        ...
        case 'SPLIT_PANE': {
            const windows = mapSessionWindows(state, (window) => {
                const shellSessionId = window.panes.find((pane) => pane.id === window.activePaneId)?.shellSessionId ?? state.shell.activeSessionId;
            ...
            }, `sim.pane.split.${action.payload}`, `Pane split ${action.payload}`);
        }
        case 'FOCUS_PANE': {
            const windows = mapSessionWindows(state, (window) => {
        ...
        case 'NEW_WINDOW': {
            ...
            }, 'sim.window.new', 'New window created');
        }
        case 'NEXT_WINDOW':
        case 'PREV_WINDOW': {
            const activeSession = getActiveSession(state);
        ...
        case 'NEW_SESSION': {
            ...
            }, 'sim.session.new', `Session ${nextSession.name} created`);
        }
        case 'KILL_ACTIVE_PANE': {
            ...
            }, 'sim.pane.kill', 'Active pane removed (if possible)');
        }
        ...
        case 'APPLY_TMUX_CONFIG': {
            ...
            }, 'sim.config.apply', errors.length === 0
        ...
        case 'ENTER_COPY_MODE': {
            ...
            }, 'sim.copymode.enter', 'Copy mode enabled');
        }
        case 'EXIT_COPY_MODE': {
            return withHistory({
                ...state,
        ...
        case 'RUN_COPY_SEARCH': {
            ...
            const matchLineIndices = query.length === 0
                ? []
            ...
            return withHistory(nextState, 'sim.copymode.search', found ? `Search match for "${action.payload}"` : `No match for "${action.payload}"`);
        }
