...
export type TmuxMode = 'NORMAL' | 'PREFIX_PENDING' | 'COMMAND_MODE' | 'COPY_MODE' | 'SEARCH_MODE';
...
export function createPane(shellSessionId: string, title = 'shell', seedLines?: string[]): TmuxPane {
    ...
    const seededTerminal = appendOutput(createTerminalBuffer({
        width: 80,
        height: 8,
...
export function createInitialSimulatorState(options?: {
    scenarioPresetId?: string;
}): SimulatorState {
