...
import { calculateLevelFromXp, calculateMissionXp, calculateNextStreak, computeCoreAchievements, computeFunAchievements, type AchievementEvaluationInput, type MissionDifficulty, } from './progressEngine';
...
export type ProgressState = {
    ...
    setProgressSummary: (payload: {
        xp: number;
        level: number;
        streakDays: number;
    }) => void;
};
type PersistedProgressSlice = Pick<ProgressState, 'xp' | 'level' | 'streakDays' | 'lastMissionPassDate' | 'completedMissionSlugs' | 'completedTrackSlugs' | 'unlockedCoreAchievements' | 'unlockedFunAchievements' | 'unlockedAchievements' | 'tmuxSkillStats' | 'missionSessions'>;
...
function startMissionSessionRecord(sessions: MissionSessionRecord[], payload: MissionSessionStartPayload): {
...
function completeMissionSessionRecord(sessions: MissionSessionRecord[], payload: MissionSessionCompletePayload): {
    sessions: MissionSessionRecord[];
    completed: boolean;
} {
    const missionSlug = payload.missionSlug.trim();
...
function buildAchievementInput(completedMissionSlugs: string[], completedTrackSlugs: string[], streakDays: number, tmuxSkillStats: TmuxSkillStats): AchievementEvaluationInput {
    return {
...
function mergeAchievementState(state: Pick<ProgressState, 'unlockedCoreAchievements' | 'unlockedFunAchievements' | 'unlockedAchievements'>, input: AchievementEvaluationInput) {
...
export const useProgressStore = create<ProgressState>()(persist((set, get) => ({
    ...
    recordMissionPass: (payload) => {
        ...
        const achievementInput = buildAchievementInput(nextCompletedMissionSlugs, nextCompletedTrackSlugs, nextStreak, state.tmuxSkillStats);
    ...
    recordTmuxActivity: (payload) => {
        ...
        const achievementInput = buildAchievementInput(state.completedMissionSlugs, state.completedTrackSlugs, state.streakDays, nextStats);
        const nextAchievementState = mergeAchievementState(state, achievementInput);
        set({
    ...
    addCompletedMission: (missionSlug) => set((state) => ({
