                ...
                if use_compression:
                    ...
                    with open(absname, 'rb') as src_file:
                        with open(to_abs, 'wb') as dst_file:
                            compress_stream(src_file, dst_file, zstd_module)
                else:
        ...
        if member.isfile() or member.islnk():
            ...
            if os.path.exists(to_abs):
                ...
                if use_compression:
                    ...
                    f.seek(0)
                    with open(to_abs, 'wb') as dst_file:
                        compress_stream(f, dst_file, zstd_module)
                else:
...
def compress_stream(src_file, dst_file, zstd_module):
    ...
    while True:
        chunk = src_file.read(128 * 1024)
        if not chunk:
            break
        out = compressor.compress(chunk, compressor.CONTINUE)
        if out:
            dst_file.write(out)

    out = compressor.flush(compressor.FLUSH_FRAME)
    if out:
        dst_file.write(out)
