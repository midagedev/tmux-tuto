...
| 기존 해금 상태 | 처리 방식 | 비고 |
| --- | --- | --- |
| 유지/통합 대상 ID가 이미 해금됨 | 대응 v1 ID를 즉시 해금 상태로 승격 | 사용자 성취 이력 보존 |
| 삭제 대상 ID만 해금됨 | 자동 해금 이전 없음, v1 evaluator 재계산 기준 적용 | 중복/저가치 업적 제거 목적 유지 |
| 여러 기존 ID가 하나 v1 ID로 통합 | v1 단일 ID만 저장 | 중복 해금 방지(idempotent) |

- 삭제 업적은 UI에 재노출하지 않고, 릴리즈 노트/변경 공지에서 "카탈로그 단순화로 통합됨"을 안내한다.
